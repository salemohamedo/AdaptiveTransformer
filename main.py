...
parser.add_argument('--adsmax', action='store_true',
                    help='adaptive softmax')
parser.add_argument('--adinp', action='store_true',
                    help='adaptive inputs')
...
model = model.TransformerModel(ntokens, args.emsize, args.nhead, args.nhid,
                               args.nlayers, args.dropout, args.adsmax, args.adinp, args.tied_weights).to(device)
...
def evaluate(data_source):
    ...
    with torch.no_grad():
        for i in range(0, data_source.size(0) - 1, args.bptt):
            ...
            output = model(data)
            if model.adsmax:
                adaptive_loss = model.decoder(output.view(-1, args.emsize), targets)
